...
This page covers the two routes every policy needs: the live page at `/legal/terms` and the
archive page at `/legal/terms/[revision]`. Both render through `PolicyDocument`, an async server
component that emits one revision's MDX body and nothing else.
...
| `components` | `MDXRemoteProps["components"]` | no | The MDX component map, forwarded to `MDXRemote`. |
...
Old revisions are the version archive, so give them a dynamic segment. The route param is
untrusted, and both guards are needed because they answer different questions: `revision()` asks
whether that revision exists at all, `content()` asks whether it carries this locale. A locale
introduced in 2027 is legally absent from the 2026 revisions, so a real revision plus a real
locale can still be a legitimate 404.
...
Neither guard throws on a miss - both return `undefined`. When you need no title from the file,
`termsPolicy.has(revision, locale)` answers both questions in one call without reading the body.
...
unguarded route param. Guard first; the throw is only the loud backstop.
...
`components` is forwarded straight to `MDXRemote`. policykit never imports `next/link`, so the
link component is the one override nearly every app needs:
...
- [Notices and consent](/notices-and-consent) - the banner, the notice queue, and the consent gate.
