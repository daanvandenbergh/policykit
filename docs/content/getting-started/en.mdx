...
resolved revision has no file for that locale. [Rendering policies](/rendering-policies/) has the
guard-then-404 recipe.

## Related

When you publish your second version, [writing a revision](/writing-a-revision/) walks the authoring
steps - or install [`/policykit`](/policykit/), the Claude Code skill that runs those steps for you.
